...
type udpResolver struct {
	...
	// timeout is the default timeout if context has no deadline
	timeout time.Duration

	// client is currently unused - kept for potential future use with miekg/dns Client API
	client *dns.Client

	// connPool is the connection pool for socket reuse
	connPool *connPool
}
...
func newUDPResolver(addr string, timeout time.Duration, poolSize int) *udpResolver {
	...
	// This allows users to specify just "8.8.8.8" instead of requiring "8.8.8.8:53".
	...
	return &udpResolver{
		...
		client: &dns.Client{
			Net:     "udp",
			Timeout: timeout,
			UDPSize: 4096, // EDNS0 UDP payload size - larger than default 512 to support bigger responses
		},
	}
}
...
func (r *udpResolver) ResolveType(ctx context.Context, host string, qtype RecordType) ([]Record, error) {
	...
	// Get a connection from the pool. This might return a reused connection or create a new one.
	conn, err := r.connPool.Get()
	if err != nil {
		return nil, fmt.Errorf("failed to get connection: %w", err)
	}

	// Set the deadline on the connection to honor context cancellation and timeouts.
	// Prefer the context deadline if set, otherwise use the resolver's default timeout.
	...
	// Send the query. If this fails, the connection is likely broken, so close it
	...
	// Read the response. Same error handling as WriteMsg: close on error.
	...
	// Query succeeded, return the connection to the pool for reuse.
	// Do this before processing the response so the connection is available ASAP.
	r.connPool.Put(conn)

	// Check DNS response code. RcodeSuccess (0) means the query succeeded.
	// Other codes include NXDomain (domain doesn't exist), ServFail (server error), etc.
	if response.Rcode != dns.RcodeSuccess {
		return nil, fmt.Errorf("dns error: %s", dns.RcodeToString[response.Rcode])
	}

	// Parse the answer section into our Record format. The DNS response contains
	// raw resource records that we need to convert into a more usable structure.
	var records []Record
	for _, ans := range response.Answer {
		...
		// Extract the value based on record type. Each DNS record type has its own
		// struct in miekg/dns, so we use a type switch to handle them appropriately.
		switch a := ans.(type) {
		case *dns.A:
		...
		case *dns.NS:
			...
			record.Value = a.Ns
		case *dns.TXT:
			// Text record - can contain multiple strings, format as a single string
		...
		default:
			// For record types we don't explicitly handle, use the library's
			// string representation. This provides basic support for any record type
			// without requiring explicit handling for each one.
	...
	// Some DNS servers return RcodeSuccess with an empty answer section when a
	// record exists but has no data (e.g., a domain with no A records). Treat this
	// as an error rather than returning an empty slice, to distinguish it from
	// never calling this function vs calling it and getting nothing.
	if len(records) == 0 {
